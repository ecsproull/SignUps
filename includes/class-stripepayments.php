...
class StripePayments extends SignUpsBase {
	...
	public function collect_money( $description, $price_id, $badge, $attendee_id, $cost, $qty, $signup_id ) {
		...
		$stripe_metadata = array(
			'su_origin'             => (string) $signup_domain,
			'su_signup_description' => (string) $description,
			'su_badge'              => (string) $badge,
			'su_qty'                => (string) (int) $qty,
			'su_amount'             => (string) ( (int) $cost * (int) $qty ),
			'su_signup_id'          => (string) $signup_id,
		);
